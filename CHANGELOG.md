...
- Ruby gem packaging: `SpreenPr` module under `ruby/lib/`, `require 'spreen-pr'` shim, `spreen-pr.gemspec`, `exe/pr-title`, and RBS signatures shipped in the gem.
- Python packaging: `spreen_pr` package under `python/src/`, full PyPI metadata in `pyproject.toml`, `pr-title` console script, and the `py.typed` marker.
